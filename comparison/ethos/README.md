...
| **Wholesale POSIX refusal** | Ethos gains purity; this tree needs `linuxclaude` to print the real binary's own version string. The Linux personality is the measured gate for real workloads, and it is a *personality* - kernel-resident today, a documented bridge to a cell - not the foundation. Purity is kept where it pays: the native API |
