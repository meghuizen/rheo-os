...
three ISAs**. Phases C-F (compute/QoS, the kernel's first interrupt + terminal,
IPC/compositor, process/time/net) are planned in docs/LIBRHEO.md.
...
tests/        in-QEMU test kernels: cap-invariants, queue-pipeline,
              ...
              columnar scan off a live virtio-blk disk), coreutils (the coreutils multicall cell, with
...
librheo/      the native userspace foundation library (docs/LIBRHEO.md):
              ...
              File/read_at/write_at/Contract)/store (Dataset) + crt0 + the
              librheo-demo (Phase A) and librheo-data (Phase B mini-DuckDB scan)
              programs
