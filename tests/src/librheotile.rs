//! In-QEMU test kernel for the tile framework (docs/TILES.md). Loads the
//! `librheo-tile` ELF into a cell with a real mapped queue pair + a minted
...
static DEMO: &[u8] = include_bytes!(concat!(
    ...
    "/../target/x86_64-unknown-none/release/librheo-tile"
));
#[cfg(target_arch = "aarch64")]
static DEMO: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../target/aarch64-unknown-none-softfloat/release/librheo-tile"
));
#[cfg(target_arch = "riscv64")]
static DEMO: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../target/riscv64gc-unknown-none-elf/release/librheo-tile"
));
...
extern "C" fn kernel_main() -> ! {
    arch::init();
    println!("librheotile: start on {}", arch::NAME);
    ...
    let entry = load::load_elf(DEMO, &mut aspace).expect("load librheo-tile ELF");
    let stack_top = load::map_stack(&mut aspace);
    let qp = load::map_queue(&mut aspace);
    println!(
        "librheotile: loaded librheo-tile ({} bytes), entry {entry:#x}",
        DEMO.len()
    );
    ...
    match outcome {
        Outcome::Exited(code) => {
            assert!(
                code == EXPECTED_EXIT,
                "librheo-tile exited {code:#x}, expected {EXPECTED_EXIT:#x}"
            );
            println!("librheotile: tile framework proof, exit {code:#x} OK");
        }
        Outcome::Faulted(addr) => panic!("librheo-tile faulted at {addr:#x}"),
    }

    println!("librheotile: PASS");
