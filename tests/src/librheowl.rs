//! In-QEMU test kernel for librheo Phase E (docs/LIBRHEO.md): the Wayland-class
//! compositor demo. Load `librheo-wl` into **two** cells, wire them with a
//! **shared typed queue pair** (one ring region mapped into both) and let them
//! pass ownership of a **sealed buffer grant** with a **flip/present
//! completion** - then assert the whole round trip on all three ISAs.
//!
//! Cell 0 is the client (role 0), cell 1 the compositor (role 1). The client
//! draws a known pattern into a buffer grant, seals it, and `commit`s it: the
//! kernel delegates the sealed grant to cell 1 read-only (zero-copy - the SAME
//! frames), the client sends the buffer handle over the shared channel and
//! switches to the server; the server maps the shared buffer, composites it into
//! its framebuffer, checksums it, and replies with the flip completion; the
//! client reaps it, asserts the compositor's checksum equals its own known
//! value (proving zero-copy cross-cell sharing), and exits `0x42`.
//!
//! Each cell also gets its own kernel queue pair (crt0 requires it); the SHARED
//! channel is a separate region at 24 GiB. This mirrors how the harness wires
//! `.user` cells - Phase E proves the connect mechanism; spawn-driven connect is
//! Phase F.
...
static DEMO: &[u8] = include_bytes!(concat!(
    ...
    "/../target/x86_64-unknown-none/release/librheo-wl"
));
#[cfg(target_arch = "aarch64")]
static DEMO: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../target/aarch64-unknown-none-softfloat/release/librheo-wl"
));
#[cfg(target_arch = "riscv64")]
static DEMO: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../target/riscv64gc-unknown-none-elf/release/librheo-wl"
));

/// The client returns this on full success (see librheo-wl.rs).
...
extern "C" fn kernel_main() -> ! {
    kernel::boot::init();
    println!("librheowl: start on {}", arch::NAME);
    ...
    let entry0 = load::load_elf(DEMO, &mut aspace0).expect("load librheo-wl (client)");
    let entry1 = load::load_elf(DEMO, &mut aspace1).expect("load librheo-wl (server)");
    ...
    // USER_CHANNEL_VA (24 GiB) - the two ends of the cross-cell queue pair.
    let channel = load::alloc_channel();
    load::map_channel_into(&mut aspace0, &channel);
    load::map_channel_into(&mut aspace1, &channel);
    println!(
        "librheowl: two cells wired; shared channel at {:#x} ({} bytes), demo {} bytes",
    ...
    let outcome = unsafe {
        ...
        // Client = cell 0 (role 0), compositor = cell 1 (role 1).
        ...
        // Start the client; its `commit` switches to the compositor and back.
        user::run(0).1
    };
    ...
    match outcome {
        Outcome::Exited(code) => {
            assert!(
                code == EXPECTED_EXIT,
                "librheo-wl client exited {code:#x}, expected {EXPECTED_EXIT:#x}"
            );
            println!(
                "librheowl: compositor demo ran (shared queue pair + zero-copy \
                 sealed buffer + flip round-trip), exit {code:#x} OK"
            );
        }
        Outcome::Faulted(addr) => panic!("librheo-wl faulted at {addr:#x}"),
    }

    println!("librheowl: PASS");
