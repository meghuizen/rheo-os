...
| Piece | Where | State |
|---|---|---|
| Funded kernel metadata (pillar 1) | `kernel/src/mm/kmeta.rs` | **Built.** `Funded<T>` grows past every old `MAX_*`, charges its owner, rolls back a failed reserve, releases exactly. **Hot-path shape (S2c):** the directory's first `INLINE_PAGES` (8) entries live in the struct itself, so every table up to 32 KiB - all of the scheduler's - resolves a page with no dependent directory load and **no directory frame charged** (the `smp` E4 oracle dropped from 2 table frames to 1, and `substrate` asserts the overflow frame appears exactly at the crossing); and scans have their own API, `page_slices`/`iter` - one resolve per page, elements by reference - because a whole-table predicate written as point access paid the page resolve *and a full copy of `T`* per element (measured 10.5 -> 4.9 instructions/element on the 64-byte shape, `funded_scan_64` vs `funded_scan_iter`). The scan consumers on decision paths are migrated: the entity table's `live_of`/`all_parked`/`create`/`leave_cpu`/`check`, the VMA list's fault-path `live`/`find`/`position`, the thread table's futex-deadline scan. The EEVDF queue deliberately keeps its `high_water`-bounded point loop over `get_ref` - its capacity is a whole page of 48-byte slots, so a full-page slice walk would *regress* a queue holding a handful of vcores; moving that bottleneck is exactly what the migration must not do. **Repeated walks are also fused, not just cheapened**: the page-fault fill asked the VMA list twice per file-backed fault (`find` then a list-level `file_at` that re-ran `find`) and now asks the record it already holds (`Vma::file_page`); the EEVDF `dispatch` walked its queue twice per decision (`eligibility_would_defer` then `pick`, identical filters) and now takes both answers from one walk with the tie rule preserved bit-for-bit; and `sched::dispatch::pick` asked the personality's runnable predicate - itself a context-table walk - up to three times per cell per decision (per queue entry in the reconcile, the final filter, the divergence probe) and now memoizes it, one snapshot of runnability per decision. Measured across the whole bench matrix rather than claimed: `p5_crosscell_roundtrip` 499 -> 483 instructions (the reschedule path's entity scans), `p2_roundtrip_batched64` -1, against +1 on `p2_roundtrip_single`/`p2_user_roundtrip`/`obs_emit_on` and +3..6 amortized on the `rng_*` draws - layout and rare-path ripples of the 64-byte-larger `Funded` struct, published rather than elided |
...
| The execution entity (pillar 3, the framework) | docs/EXECUTION-MODEL.md | **Designed, not built.** Pillar 3 was built incrementally and five defects came out of one cause: an execution entity has three representations, and the agreement between them is maintained by hand at 8 predicate sites and 4 claim sites. EXECUTION-MODEL.md is the top-down design - one entity table with a 64-byte hot line, the state and ordering dependency graphs, ten invariants, a use-case simulation that names the four blocked capabilities and the missing field behind each, a host fuzzer over the state machine, and FRED behind observation. It owns the rest of pillar 3 |
