...
| **E4** | Per-entity funded FP area; `MAX_VCORES` no longer the resource limit | frames measured per context, and returned with the slot | **the binding constraint is done**. Each entity funds one frame from its own cell's budget, so `MAX_VCORES` went 4 -> 16 with the `.bss` cost *falling*; `smp` measures the per-context frame cost and the return, `verify/entity` checks fund-once/distinct/release-once. Remainder: moving `vframe`/`vqp`/`vqp_va`/`vqp_cap`/`voutcome` onto the entity too - about 11 KiB of `.bss` at 16 contexts against the 1 MiB the FP areas alone were, so the constant is now an array bound rather than a resource limit. Deleting it **outright** additionally needs the *native* id to stop being derived, because a derived id is a stride and a stride bounds contexts per cell - the Linux side already allocates instead (docs/EXECUTION-MODEL.md 9.1/9.2), so the pattern to follow exists |
