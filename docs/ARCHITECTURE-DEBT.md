...
| ~~`linux::pipe::PIPES` **1,048,960 B**~~ - **done** (docs/EXECUTION-MODEL.md 9.8): the ring is `Funded<PipePage>` reserved at open and released when the last end closes, charged to the cell that opened it; the symbol is gone from the ranking. `Elastic` did not apply - a whole `Pipe` is 65,552 B and a `Funded` element must fit one frame - so the buffer became the funded thing rather than the record. **Three still fixed**: `linux::LINUX_STATE` 427,776 B, `linux::inetsock::DGRAMS` 131,520 B, `linux::signal::ACTIONS` 33,280 B | **G**. Found by `cargo xtask sizes`, not by reading constants - the ceiling-removal work up to this point had been aimed at the `MAX_*` values already named in docs, and `PIPES` is larger than every one of them combined while carrying no `MAX_*` name at all. `SYSCALL_KSTACK` (524,288 B) and `frames::REFS` (131,072 B) sit in the same list and are **not** this defect: per-CPU kernel stacks and one refcount byte per frame are real, sized by the hardware |
