...
| **The kernel's four largest statics are still fixed arrays**, now measured rather than guessed at: `linux::pipe::PIPES` **1,048,960 B** (16 pipes x a 64 KiB buffer each, resident whether or not a pipe is ever opened), `linux::LINUX_STATE` 427,776 B, `linux::inetsock::DGRAMS` 131,520 B, `linux::signal::ACTIONS` 33,280 B. All four are the `Elastic` shape (docs/EXECUTION-MODEL.md 9.6) | **G**. Found by `cargo xtask sizes`, not by reading constants - the ceiling-removal work up to this point had been aimed at the `MAX_*` values already named in docs, and `PIPES` is larger than every one of them combined while carrying no `MAX_*` name at all. `SYSCALL_KSTACK` (524,288 B) and `frames::REFS` (131,072 B) sit in the same list and are **not** this defect: per-CPU kernel stacks and one refcount byte per frame are real, sized by the hardware |
