...
| **Per-CPU feature divergence** (a hybrid part; cores without an FPU or a vector width their siblings have) | a CPU that lacks `FloatSimd` is asserted not to offer it | **G**. `graph_build` currently asserts the *machine's* features of every CPU and says so at the site - true of every profile this runs on, and the placeholder the row below depends on. The per-CPU *class* half is now done (below), and it established the mechanism this needs: only a core can read its own CPUID, so a secondary classifies itself at bring-up |
