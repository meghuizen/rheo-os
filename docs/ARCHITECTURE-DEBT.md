...
| **E2** | Ownership + the entered-guard into the table | the predicate and claim sites delegate | **done**. `RunCell.vcpu[]` and the per-CPU entered-guard are gone; the entity id **is** `cell * MAX_VCORES + vcore`, so there is no mapping to drift. `leave_cpu` is keyed on the CPU because a cross-cell chain means the entity a core returns from is not the one it entered. Both halves of the guard observed load-bearing; the child-inherits-owner revert still does not fire, exactly as docs/SMP.md 10.0c already records |
| **E3** | Runnability into the table; the personality stops keeping copies | I4 asserted in a real boot | **done, both halves**. Native: `nproc`'s `vparked[]` is gone - `parked()` reads the entity, park/wake write it, `all_parked` is the table's own implementation - and **I4 becomes checkable**, since "parked with no wake source" is a state a personality-side boolean could not express. Two controls firing (one hangs the boot). Linux: `Thread::state` is gone too, `TState` surviving only as a view computed from the entity, with the id **allocated above the derived band** and stored on the thread exactly as docs/EXECUTION-MODEL.md 9.1 decided (the floor enforced in `EntityTable::init`/`create`, not by the caller, so a `create_at` cannot overwrite it silently); `release_cell` and `reset` became release paths and `clone` gained `-EAGAIN` when no entity can be funded. Proven by `linuxthreads` on all three ISAs **across** the teardown rather than after it, because the harness resets at the start of a run - control fires (`1 of 1 Linux context entities survived`). **And the cell-level copy is gone too** (docs/EXECUTION-MODEL.md 9.5): `PState`'s `Runnable`/`Blocked` were a cache of the contexts' states, written by a wake scan before every pick, and a stale cache there is a hang or a spin. The enum is `Free | Live | Zombie` - the lifecycle, the part no entity can answer - and runnability is derived (`Live && (any context Ready || any parked context satisfiable)`), so the scan and the `state = Blocked` write are both deleted. Proven by the whole Linux suite including the three strict production-runtime gates, which is the evidence that counts since this is the predicate Node/Bun/Claude Code are picked by; control fires (dropping the `satisfiable` half deadlocks `linuxproc`). That work found a **second** defect: a Linux cell held **two entities for one context** - `init_cell` allocated one for context 0 beside the vcore-0 entity `install` had already made for the same frame - invisible until 9.4 gave both one id space; context 0 adopts now, one creator and one releaser. Honest: I4 is vacuous in this kernel (nothing is left parked; `verify/entity` exercises it), and the adoption is a **non-result as a control** - `any_ready` scans the cell rather than calling `all_parked`, so restoring the duplicate breaks nothing else, and `linuxthreads` therefore asserts it directly instead of leaving a cleanup nothing checks |
| **E4** | Per-entity funded FP area; `MAX_VCORES` no longer the resource limit | frames measured per context, and returned with the slot | **the binding constraint is done**. Each entity funds one frame from its own cell's budget, so `MAX_VCORES` went 4 -> 16 with the `.bss` cost *falling*; `smp` measures the per-context frame cost and the return, `verify/entity` checks fund-once/distinct/release-once. **The five per-vcore arrays are gone too** (docs/EXECUTION-MODEL.md 9.3): they were **704 of `RunCell`'s 840 bytes**, and five arrays indexed by the same number are one record - vcore 0 inline, the tail a `Funded<Vcore>` charged to the cell, so the common single-context case allocates nothing and a cell that wants more pays one page for ~85. The obvious design was **simulated and refused** first: one funded table per cell costs 256 KiB of frames to save 21 KiB of `.bss`, because a `Vcore` is 48 bytes and a frame is 4096. Measured `RunCell` 840 -> 184; the *predicted* hot-path win did not appear and is recorded as a wrong prediction (icount flat - the compiler was already reading fields, not copying the struct). The `smp` phase now measures the two costs separately - first context 1 area + 2 table frames, each one after it exactly 1 - because one batch could not tell "the table is amortised" from "every context allocates a table". **And `MAX_VCORES` is gone** (docs/EXECUTION-MODEL.md 9.4). It bounded four things, and the largest was hiding: `CELL_FP` was still `[FpArea; MAX_CELLS * MAX_VCORES]` - **the 1 MiB E4 exists to remove, paid in full** as a fallback after the areas became funded; it is a *single* counted area now, single because sharing one between two live contexts is the `SYS_YIELD` FP defect rather than a degraded mode. The id **stride** was the real ceiling - and the same arithmetic was written a second time in `smp`'s placement queue, so the derivation had already failed at the one job it did; ids are allocated and stored in the context's own record, the queue carries them, and E3's two-ways-to-obtain-an-id collapses to one (`create_at` and the reserved band deleted). `nproc::Proc` got the 9.3 treatment too, 656 -> 56 bytes. Reserving id 0 as "no context" found a latent defect **`verify/entity` caught rather than a boot**: `Funded` grows by whole frames and an all-zero `Entity` is not `Entity::EMPTY` (`owner`/`inside` want `u16::MAX`), so a slot grown past but never written read as "free, owned by CPU 0" - I7 fired on nine scenarios, and `create` now initialises every slot a growth adds. Measured: per-cell scaffolding `.bss` 21,504 -> 1,408 B, `CELL_FP` 1,048,576 -> 4,096 B. The one surviving per-context bound is a **real** one and says so where it lives (`load::MAX_QUEUE_VCORES`: a mapped ring needs address space, and the window is 4 GiB). Proven by `smp` with **25 contexts on one cell** costing 26 frames and returning 27, on all three ISAs, plus the entity round trip in both directions; four controls observed firing |
...
| **Per-CPU feature divergence** (a hybrid part; cores without an FPU or a vector width their siblings have) | a CPU that lacks a feature is asserted not to offer it, and the machine stops advertising it | **done**. `CpuInfo.features` is per CPU, read by that core; the machine advertises the **intersection** (the early Alder Lake rule made mechanical) with the union kept beside it, and the graph's per-CPU `IsaSet` keeps each core's real set so a pinned placement can still use it. `hwinfo` proves the rule under a declared divergence and asserts that a core which never started reports **nothing rather than a copy**; `smp` asserts the four online cores read their own and agree. Three controls firing - and a fourth recorded: breaking either `set_isa` site alone changes no answer, because the two are one claim and each corrects the other |
| **Core classes (P/E/LP) + capacity + Intel Thread Director** | `verify/hetero/`: 8 properties over declared machines and 20,000 random ones; `hwinfo`: the discovery ran and honestly found nothing | **done** for the model, the discovery and the placement decision (`sched::hetero`: class from observed relinquish behaviour, fastest-core for compute/unknown, slowest for bursty, mismatched steals counted not prevented, fairness deliberately not rescaled). **QEMU models no hybrid part** - no CPUID leaf `0x1A`, no hybrid flag, no `capacity-dmips-mhz`, all read out of its source - so consumption is gated on a *declared* asymmetry with its own `ClassSource::Declared`. Four controls firing; one that did not fire is recorded (the uniform-machine gate in `pick_cpu` was redundant against the tie rule, and was removed rather than kept). Also wired into the **multi-core claim** (`place_cells_classed` + `claim_matching_tier`, safe by the same `PLACE_RUN` exchange `steal` uses) and proven by the `smp` kernel on all three ISAs under a declared two-tier machine: every compute cell on a full-capacity core, every bursty one on a reduced core, all claims through the preference, zero tier crossings. Still open: Thread Director's read path (needs the hardware), and per-CPU feature divergence |
...
| `/sys/devices/system/cpu` synthesis | an unmodified binary reads `cpuN/topology/{core_id,physical_package_id,thread_siblings_list}` and gets what `-smp` declared | **done** - `cputopo` in `linuxsmp`, all three ISAs, synthesized from the same discovery the graph is built from and **refused** rather than defaulted when the topology is unknown. Two controls firing (a constant `core_id`; the index bound removed, which is a kernel panic from a cell-chosen path). Still absent and named: the `cache/index*/` tree, because this kernel discovers who *shares* a cache and not how big it is |
| `/sys/devices/system/node` synthesis | hwloc reads the memory localities too | **done** - `numatopo` in `linuxsmp`: `online`, `nodeN/cpulist`, `nodeN/distance`, `nodeN/meminfo`, with QEMU's two `-numa node` lines and `dist,val=20` as the oracle. x86-64 and riscv64 read two nodes with the declared distances; ARM64 reads the one node it genuinely has, asserted rather than skipped. `MemFree` deliberately absent, a nonexistent node refused. Two controls firing |
...
| Work stealing within an LLC domain, crossings counted | a steal prefers the cache domain; the crossing count is asserted | **P (E3/E4), and the blocker is not discovery** - `siblings(cpu, Cache)` exists. Examined and refused for now: `smp::steal` moves only cells that have **not started**, so nothing of them is in the victim's cache and there is no transfer to optimise, and the strand injector is one shared queue with no victim to prefer between. Per-entity run state with migration is what creates a warm victim. Reasoning in docs/RESOURCE-GRAPH.md 6.3a |
...
| **Contract-checked channels** (Singularity) | an out-of-order message is a *compile* error; the wire format is unchanged | **G** - fills `idl/`, which is a stub. Ethos adds one requirement to this row (comparison/ethos/ 2): the IDL also emits a **runtime type hash** per protocol, because a compile-time contract is single-language and the boundary check must protect a Rust cell from a C cell |
| **Channel protocol identity** (Ethos Etypes) | a channel minted with protocol hash A, connected to by a cell expecting B, is refused at `SYS_CONNECT` - not answered with `REPLY_NONE` mid-protocol | **G** - one word on `ChannelInfo`, set by the launcher (the authority that already mints the queue pair), checked in `netservice`; the same field-extension precedent IDENTITY.md 4 plans for peer credentials |
| **Cell image measurement** (Ethos auth; IDENTITY.md ID2) | the kernel-reported hash of a loaded cell equals an xtask-computed sha256 of the same ELF - an oracle the kernel cannot fake | **G** - needs the one missing piece, a zero-dependency kernel SHA-256 (the kernel today has only an inline FNV receipt and ChaCha20); measure at `load`, report beside the engine attach measurement it is the sibling of |
...
| ~~**`/proc/stat` has one `cpuN` line whatever the boot's CPU count is**~~ | **the line count is done; the jiffy fields are named, not fixed.** `/proc/stat` predates the sysfs tree and counting its `cpuN` lines is what every libc's `get_nprocs` falls back to, so a program taking the portable route was told there was one core while `online` said four. The count comes from `smp::online_count()` now and the seeded file is **gone**, so a constant cannot answer first - the `online` fix applied to the same defect one file along. Proven by `procstat` in `linuxsmp` against **QEMU's `-smp 4`**, not against the kernel's own count; control observed firing (`n = 1` restored: `cpus=1`). The ten jiffy fields stay **0**, and that is the honest answer rather than a placeholder: `sched::dispatch` charges a vcore the ns it ran, aggregated across CPUs with no user-versus-kernel split, so there is nothing to convert into them. Splitting the charged time across them would invent a breakdown, which is worse than a zero - a reader seeing 0 learns "no accounting", where a reader seeing a fabricated `user` figure computes a CPU percentage from it. Remaining: **G** - per-CPU user/system time accounting |
...
| ~~**`linux::proc::preempt_cell` is unexercised**~~ | **done.** The shape that reaches it is a fork where **both** sides are compute-bound at once - two single-context cells, neither with a sibling to move to, and cooperatively the child cannot run at all until the parent reaches `waitpid`, so the fixture (`preemptfork.c`) spins in the parent *before* waiting. The last `linuxproc` phase runs it twice: with dispatch off, **0** cross-cell preemptions (the control - without it an interleave could be evidence of preemption or of the two having been interleaved all along), then with the slice armed, 392-930 of 392-930 preemptions to the *other cell* and **0** to a sibling context, on all three ISAs. The transcript is byte-identical either way, which is the correctness half: preempting a Linux cell at an arbitrary instruction inside its own compute loop changed nothing it computed. Control observed firing (`preempt_cell` returning `None`: `0 taken` in 1975 armed slices) |
| ~~Console buffering is never enabled in a boot~~ / ~~`telemetry` has no in-kernel user~~ | **done.** The `preempt` kernel enables the buffered console for one phase on all three ISAs: 5 distinct lines take 5 slots, 8 identical ones fold into 1, a ring overflow is **counted and reported** rather than silently overwriting, the flush drains every record, and buffering is off again afterwards so the rest of the boot is byte-for-byte what it was. Two controls firing (the console wiring removed; coalescing removed). It also found that `Rings::bypassed` is unreachable from the console - `console::write` short-circuits before it, deliberately - so the counter belongs to a direct API caller and now says so, with the phase asserting *that* path instead |
| ~~`netdns`'s live phase could fail on what a real resolver returned~~ | **fixed, and the attribution is inferred rather than observed.** A full matrix failed once at kernel 53 (`netdns`) and passed on the next three runs; the failing log was overwritten before it was read, so *which* check fired was never seen. What reading the phase found is a genuinely reachable non-deterministic failure in something documented as network-tolerant: every `Err` was tolerated, but an `Ok` carrying no A record - an AAAA-only or empty answer from a real upstream - fell into `fail(70)`, and a zero-TTL answer into `fail(71)`. Both are now reported, so the live phase asserts only what cannot vary (a query was sent). Honest: this is the only network-dependent assertion in that position and the only plausible cause found, but it is a diagnosis by reading rather than by reproduction |
| ~~`linux::pipe::PIPES` **1,048,960 B**~~ - **done** (docs/EXECUTION-MODEL.md 9.8): the ring is `Funded<PipePage>` reserved at open and released when the last end closes, charged to the cell that opened it; the symbol is gone from the ranking. `Elastic` did not apply - a whole `Pipe` is 65,552 B and a `Funded` element must fit one frame - so the buffer became the funded thing rather than the record. **Two more done** (9.8): `inetsock::DGRAMS` **131,520 B** -> the datagram queue is `Funded<Datagram>` reserved on bind and released on close (gone from the ranking); and `FdTable::maps`, the 8 KiB-per-cell `/proc/self/maps` snapshot, is funded on first read, taking `LINUX_STATE` 427,776 -> **296,704 B**. **Examined and refused**: `signal::ACTIONS` (33,280 B) is `[[SigAction; NSIG+1]; MAX_CELLS]`, and `NSIG` is the number of signals the Linux ABI defines - a dense, fixed-by-contract array, not a resource limit, so it is the `MAX_CELL_CHANNELS` category. **And the fd paths too**: `FdKind::Vfs`'s inline `[u8; PATH_MAX]` moved to a per-cell funded table indexed by fd slot, taking `LINUX_STATE` to **34,560 B**. That created an obligation `fork` did not have - `dup_state` raw-copies `LinuxState`, so the child inherited `path_len` with an empty path table and would read a zeroed path with no fault and no log - and **nothing in the suite noticed** (deleting the copy left `linuxproc`/`linuxtools`/`linuxdyn` green), so `forkdir.c` was written: a directory fd inherited across `fork` and used by-fd in the child. Control fires. What remains at the top of the ranking is closed **with a structural reason**, not blessed: `SYSCALL_KSTACK` (524,288 B = 8 slots x 64 KiB) is a **kernel stack**, so it needs contiguous memory, and `Funded` is deliberately a *directory of non-contiguous frames* - `mm::kmeta`'s own doc records that adding a contiguous allocator would make table growth fail on fragmentation, the worst failure mode for kernel metadata. `frames::REFS` (131,072 B) is one refcount byte per pool frame, so funding it *from* the pool is circular. Neither is the array-dimension defect; both would need a different mechanism, and neither should be re-opened without one | **done** | Found by `cargo xtask sizes`, not by reading constants - the ceiling-removal work up to this point had been aimed at the `MAX_*` values already named in docs, and `PIPES` is larger than every one of them combined while carrying no `MAX_*` name at all. `SYSCALL_KSTACK` (524,288 B) and `frames::REFS` (131,072 B) sit in the same list and are **not** this defect: per-CPU kernel stacks and one refcount byte per frame are real, sized by the hardware |
| **riscv64's DRBG root is seeded from a documented fallback, not from entropy** - measured, not assumed: the `rng` kernel reports `hwrng=none (Zkr seed CSR needs M-mode mseccfg grant) present=false root_seed_source=Fallback` there, against `RDSEED`/`Hwrng` on x86-64 and `RNDR` on aarch64. QEMU's riscv `virt` **does** offer `virtio-rng-device`, so the gap is a missing driver rather than a missing device | **G**. The `claude/os-design-euroos-comparison` branch has a 495-line `hw/virtio_rng.rs` that closes it. That branch is **not mergeable** (below), but this one file plus a seed-source wiring is a self-contained slice worth taking on its own |
| **Branch triage** (assessed by trial merge, not by inspection). `claude/gpu-driver-support-arch-vnz97p`: 0 commits ahead, 203 behind - fully superseded, **absolve**. `origin/main`: merged; its four SMP start-all commits are superseded by this branch's further work (a stack per hart indexed by its own hardware id, versus main's single sequential `secondary_sp`), so the conflicts resolved to ours; what it genuinely brought was `tutorials/nl/`. `claude-md-test-pipelines`: **refused wholesale** - it would have introduced **5 `#[cfg(feature = "smp")]`-gated locks** in files that merged *cleanly*, silently regressing the documented decision that locking is a property of a data structure and not of a build configuration; its npm module-resolution work was **ported** instead. `os-design-euroos-comparison`: **refused** - 228 lines of conflict in the RNG core between its credited entropy pool and this branch's per-CPU root DRBGs, which is a design slice rather than a merge | recorded |
