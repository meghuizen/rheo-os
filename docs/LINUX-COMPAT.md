...
| readlink of `/proc/self/exe` | full | **Fixed for the initial process.** The path was set only by the `execve` *syscall*, so `/proc/self/exe` resolved for a process another cell had exec'd and returned `ENOENT` for the one the launcher loaded - and the test covering it exercised the working side (docs/ENGINEERING.md 11). `install_cell` now takes the exe path as an **explicit argument**, because it cannot be derived: an in-memory image genuinely has none, and passing `b""` there is the truthful answer rather than a guess. Found by the real Bun binary aborting in `createFakeTemporaryNodeExecutable`; five hypotheses were eliminated first, and what ended it was a **diagnostic** - a refused `readlink` now names its path, the way a refused `open` already did |
| stat, lstat (x86-64 legacy) | full | Routed to `newfstatat(AT_FDCWD, path, buf, 0)` - the same call; no symlinks exist in this VFS, so `lstat` cannot differ from `stat`. The **third** instance of the two-numbers hazard after `open` below and `readlink`. Found by Bun's trace showing `ENOSYS nr=4`: programs that issue these numbers are the ones bypassing libc (Bun's Zig runtime does), because **this glibc routes `stat()` through `newfstatat` even on x86-64**. That matters for how it is proven - the first fixture called `stat()` and passed with the fix reverted, so the `lstatx` fixture issues the raw syscalls instead (docs/ENGINEERING.md 1: a proof that cannot fail is not a proof). An unreachable sentinel on the asm-generic tables |
