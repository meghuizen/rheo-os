...
fn gpu_device_args(arch: Arch) -> Vec<String> {
    ...
    if arch == Arch::X86_64 {
        // AMD (ati-vga), Bochs, VMware SVGA and Red Hat/QXL are x86-only in QEMU.
        // ati-vga requires vgabios-ati.bin and bochs-display requires
        // vgabios-bochs-display.bin ROM files, only available in x86-64 environments.
